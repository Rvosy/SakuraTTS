...
def prepare(args):
    ...
    files = sorted(set(source["source_sha256"]) | {"research/tools/native_cancellation.py"})
    for name in files:
...
def execute(args):
    ...
    command = [sys.executable, str(run / "source/research/tools/native_cancellation.py"), "worker", "--run", str(run)]
