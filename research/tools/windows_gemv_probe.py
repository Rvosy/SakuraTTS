...
def probe(args, weights, model_identity, inputs, report):
    from sakuratts.backends.cuda.gpt import _GraphBLAS
    import cupy as cp
...
def main(argv=None):
    ...
    try:
        if not args.run_gpu:
            require("cupy" not in sys.modules and "sakuratts.backends.cuda.gpt" not in sys.modules, "CPU check unexpectedly loaded CUDA backend")
