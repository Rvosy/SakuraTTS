...
def worker(args):
    ...
    report = dict(status="running", scope=prepared["scope"], config=config, command=[sys.executable, *sys.argv],
        ...
        timing_scope=("Diagnostic synchronized boundaries included; do not use for speed" if diagnostic else
                      "No diagnostic boundary sampling; per-request frontend and synthesis load/hash/release plus computation included, including lazy pyopenjtalk imports; initial research/tools/runtime imports, preparation resource hashes, reference/gold loading, validation and file writes excluded"),
...
def execute(args):
    ...
    command = [sys.executable, str(run / "source/research/tools/native_text_speech.py"), "worker", "--run", str(run)]
