...
def prepare(args):
    ...
    files = sorted(set(source["source_sha256"]) | {"research/tools/native_staged_lifecycle.py",
        "research/tools/japanese_frontend_process.py", "research/tools/process_tree_rss.py"})
    for name in files:
...
def worker(args):
    ...
    from sakuratts._internal.synthesis import prepare_text, generate_prepared_semantic, synthesize_acoustic
...
def execute(args):
    ...
    command = [sys.executable, str(run / "source/research/tools/native_staged_lifecycle.py"), "worker", "--run", str(run)]
