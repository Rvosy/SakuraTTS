...
`tools/convert_sovits_onnx_fp16.py` 创建候选，并将验证初始状态设为失败；`research/tools/windows_acoustic_precision.py` 完成工程筛查后才写入绑定图、权重和执行设置的结果。当前通过的候选位于 `outputs/windows-acoustic/fp16-candidate-lowered`。
...
```powershell
.venv/Scripts/python.exe -B tools/convert_sovits_onnx_fp16.py `
...
.venv/Scripts/python.exe -B research/tools/windows_acoustic_precision.py `
