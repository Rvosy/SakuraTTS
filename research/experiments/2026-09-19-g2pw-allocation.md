...
`research/tools/g2pw_prepacking.py` 为每个策略创建独立进程，先保存概率与正确性比较，再关闭捕获，对五组实际规范化中文输入各预热两次、计时五次。计时区间只有实际 `predict`，包含去重、模型执行及标签 / 置信度读取；文件复制、哈希、数组比较和 RSS 查询在计时外。两个候选都只改 ORT 一个选项，自有文本或模型能力未削减。
