...
原始两条文本已逐字保存到 `benchmarks/cases/speech_regressions.json`，另加 8 条短句、长句、标点和混合文本。新样例尚未获得内容质量验收。自动检查、ASR、人工试听分别记录；本轮初始定位时未执行 ASR 或人工试听；后续结果见 [ASR 与用户复听](2026-09-19-asr-review.md)。
...
前端原始记录保存在 `../SakuraTTS-References/runs/20260919-frontend-audit/`，含 CPU 脚本、音素、OpenJTalk 节点、BERT 输入与特征。控制流实验是 `research/tools/audit_lite_controls.py`，证据目录为 `runs/20260919T103023.000057Z-lite-control-audit-cpu/`。
...
`research/tools/gpt_fixed_history.py` 只加载 GPT，用官方 x、参考 token、BERT 和每一步历史重放 Lite 的 Prefill 与 Decode。上述全部 1025 维 logits 逐元素一致，top-1 也全部一致。这两条样例没有发现 GPT 权重映射、位置编码或缓存解码计算差异。该结论不覆盖自由采样规则、SoVITS、其他精度或其他模型。
...
`research/tools/prepared_reference.py` 使用官方原始方法准备参考 token、频谱、参考文本特征与说话人 embedding，保存这些条件，然后释放 CNHuBERT 和 ERes2Net。后续请求复用完全相同的说话人 embedding。该适配器只接受这一个参考张量，换参考时明确报错，尚不是通用缓存加载器或正式运行时。
...
"$REF/.venv-macos/bin/python" -u research/tools/reference_smoke.py --references "$REF" --backend lite --device mps --repeat 1 --diagnostic
"$REF/.venv-official-macos/bin/python" -u research/tools/reference_smoke.py --references "$REF" --backend official --device mps --repeat 1 --diagnostic
"$REF/.venv-macos/bin/python" research/tools/audit_lite_controls.py --references "$REF"
"$REF/.venv-macos/bin/python" research/tools/gpt_fixed_history.py --references "$REF" --official-run "$REF/runs/20260919T102853.549769Z-official-mps" --device mps
"$REF/.venv-official-macos/bin/python" -u research/tools/reference_smoke.py --references "$REF" --backend official --device mps --repeat 4 --prepare-reference
```
