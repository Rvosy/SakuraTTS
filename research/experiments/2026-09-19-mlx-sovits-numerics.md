...
`research/tools/mlx_sovits_numerical_diagnosis.py` 在独立进程里，用 NumPy FP64 对原始 `g/v` 归一化，再一次转换成 FP32。分别替换 decoder、flow 和两者的归一化结果，计算过程仍为 MLX FP32。诊断保留原始 `g/v`，不会改写转换包或默认运行模块，也没有按输入或采样点作特殊处理。
...
"$REF/.venv-mlx-macos/bin/python" research/tools/mlx_sovits_numerical_diagnosis.py \
  --references "$REF" \
