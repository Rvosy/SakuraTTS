...
`research/tools/attention_node_diagnosis.py` 直接读取官方逐层捕获中的 `ssl.0.input`，形状为 FP32 `[1,192,422]`，不重新生成语义 token。模型仍是朱雀院红叶 V2Pro，官方提交为 `48b1a0169a28582a8984402f82cf438d3bfa6aca`，权重使用原 FP32 转换包。该层为两头、每头 96 维、相对位置窗口 4；输入没有 padding，mask 全为 1。
...
"$REF/.venv-official-macos/bin/python" research/tools/attention_node_diagnosis.py \
  --references "$REF" \
