...
`tools/prepare_g2pw_ort.py` 在新目录生成文件，不改原模型。`G2PWSession.from_ort_package()` 核对 ORT 版本、build、系统、架构、CPU provider 和模型 SHA-256，再启用 `session.use_memory_mapped_ort_model` 与 `session.use_ort_model_bytes_for_initializers`。重载使用 `ORT_DISABLE_ALL`，因为图优化已保存。Session 存活时不能改写映射文件。
...
"$REF/.venv-mlx-macos/bin/python" tools/prepare_g2pw_ort.py \
  --source <原始g2pW.onnx> --output-root "$REF/models/converted"
"$REF/.venv-mlx-macos/bin/python" research/tools/g2pw_prepacking.py \
  --equivalence-run "$REF/runs/20260919T125451.773385Z-g2pw-session" \
  --policies default runtime-mmap --optimized-package <新ORT包>
"$REF/.venv-mlx-macos/bin/python" research/tools/g2pw_lifecycle.py \
  --equivalence-run "$REF/runs/20260919T125451.773385Z-g2pw-session" \
  --ort-package <新ORT包>
```
