...
新 Harness 为 `research/tools/sovits_static_weights.py`，每个配置独立进程运行，支持 CPU/GPU Flow 与 Decoder，声学 Encoder 使用 CPU，并显式记录已有 softmax 选项。
...
"$SOVITS_PY" research/tools/sovits_static_weights.py --references "$SOVITS_REF" --package "$SOVITS_PACKAGE" --official-conditions "$SOVITS_CONDITIONS" --mode diagnostic --device gpu --encoder-softmax fp64-accumulation --no-fold-weight-norm

"$SOVITS_PY" research/tools/sovits_static_weights.py --references "$SOVITS_REF" --package "$SOVITS_PACKAGE" --official-conditions "$SOVITS_CONDITIONS" --mode diagnostic --device gpu --encoder-softmax fp64-accumulation --fold-weight-norm --equivalence-reference "$SOVITS_UNFUSED_RUN"
```
