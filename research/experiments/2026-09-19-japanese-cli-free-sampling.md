...
`tools/synthesize_japanese.py` 已用原始日文、独立模型包和新准备的参考条件完成三次生成。seed 0、seed 1 各运行一个独立进程，再用第三个进程重复 seed 0；三次均退出 0，并由 EOS 停止。运行过程没有读取历史目标特征、目标 token、随机噪声或对照 trace。
...
三个进程结束时均检查到以下模块未导入：`torch`、`transformers`、`sakuratts.frontend.chinese`、`sakuratts.frontend.g2pw`、`sakuratts.backends.mlx.bert`。
...
"$tts_refs/.venv-japanese-macos/bin/python" "$tts_source/tools/synthesize_japanese.py" \
