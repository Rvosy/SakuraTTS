...
`scripts/build_preview.py` 从明确的源码目录构建 wheel / sdist，再生成 ZIP、源码哈希与产物校验清单。它拒绝覆盖已有输出，不上传文件。归档生成前，逐文件检查 sdist 是否包含完整选中源码，以及 wheel 的产品代码和许可证是否与源码快照一致。首次候选漏掉两份 `benchmarks/cases/*.json`，已修复 MANIFEST；该失败记录仍保留。
