...
`research/tools/softmax_candidates.py` 不加载模型，直接读取官方首层的 `[1,2,422,422]` 分数。四条路径均使用相同输入，没有改变 mask 或筛选元素。
...
`research/tools/encoder_softmax_candidate.py` 在独立进程里替换声学 attention 函数，只改变 softmax。候选统一覆盖 SSL、text、MRTE 和 encoder2，不按层、语言、句子或坐标选择路径。LayerNorm、线性运算、flow、decoder、模型权重、固定语义 token、音素、参考条件和噪声保持原样。
...
`research/tools/native_prepared_speech.py` 增加同名 `--encoder-softmax` 选项，并将配置写进结果清单，供准备条件整链验证选择。此次源码迁移先用固定声学条件复测：
...
"$REF/.venv-mlx-macos/bin/python" research/tools/softmax_candidates.py \
  --references "$REF" \
...
```sh
"$REF/.venv-mlx-macos/bin/python" research/tools/encoder_softmax_candidate.py \
  --references "$REF" \
...
将最后一项改为 `--candidate mlx-fp64`，即可复现通过十例的 MLX FP64 累积候选。其普通请求成本由独立的 `research/tools/encoder_softmax_benchmark.py` 测量：两次热身、五次计时，编码器与完整声学请求分别运行，关闭 capture，并将数组复制、比较、哈希和内存轮询放在计时之外。
