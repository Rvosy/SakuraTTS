...
`research/tools/windows_lite_benchmark.py` 检查固定 checkout、保存运行时依赖和源码哈希、禁止在线下载，分别记录完整返回、容量是否耗尽、语义 token、裁头、PCM 及资源边界。`--preflight-only` 在隐藏 CUDA 后检查依赖和日文前端，本轮通过且没有初始化 CUDA。独立环境的 `uv pip check` 通过。

```powershell
outputs/reference-research/lite-runtime/Scripts/python.exe -B research/tools/windows_lite_benchmark.py `
