...
def worker(job_file):
    ...
    sys.path[:0] = [str(root), str(root / "GPT_SoVITS"), str(PROJECT / "src")]
    os.chdir(root)
    ...
    try:
        ...
        try:
            ...
            from sakuratts.reference_condition import PreparedReference, validate_arrays
        ...
        fast_langdetect.infer._default_detector = fast_langdetect.infer.LangDetector(
            fast_langdetect.infer.LangDetectConfig(cache_dir=Path(job["output"]) / "frontend"))
...
def main():
    ...
    if args.frontend_preflight:
        ...
        return 0
    if not all((args.official_source, args.character, args.output)):
        parser.error("--official-source, --character and --output are required")
    ...
    root, character, output = args.official_source.resolve(strict=True), args.character.resolve(strict=True), args.output.resolve()
    ...
    frontend = prepare_frontend(root, output / "frontend", args.language_model, preflight)
    print("PREPARED_FRONTEND", output / "frontend", flush=True)
    ...
    protected_paths = [character / "character.json", *inputs.values(), *(Path(ref["audio"]) for ref in references)]
    ...
    protected_paths.extend(p for p in (root / "GPT_SoVITS/pretrained_models/chinese-hubert-base").rglob("*") if p.is_file())
    ...
    job = {"official_source": str(root), "output": str(output), "gpt": str(inputs["gpt"]), "sovits": str(inputs["sovits"]),
           "source_id": frontend["official_commit"], "device": args.device, "precision": args.precision,
    ...
    if returncode:
        ...
        print(subprocess.list2cmdline([sys.executable, "scripts/convert_gpt.py", "--checkpoint", str(inputs["gpt"]),
              "--official-source", str(root), "--output", str(output / "gpt")]))
        print(subprocess.list2cmdline([sys.executable, "scripts/export_sovits_onnx.py", "--checkpoint", str(inputs["sovits"]),
