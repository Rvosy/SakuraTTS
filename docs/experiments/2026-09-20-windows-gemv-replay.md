...
`harness/windows_gemv_replay.py` 复用 `windows_gpt_precision.replay`，逐一注入官方 capture 中的原始 token。每条历史产生一行 Prefill logits 和全部 Decode logits；最后一个已采样 token 不再喂入 Decode。短句、长句、多句、标点分别为 44、676、259、203 行，共 1182 行、1178 次 Decode。参考条件、文本、KV 容量 2048 和 baseline attention 均保持不变。
...
```powershell
.venv-windows-runtime/Scripts/python.exe -B harness/windows_gemv_replay.py `
