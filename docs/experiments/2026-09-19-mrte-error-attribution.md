...
`harness/mrte_numerical_diagnosis.py` 读取官方和 MLX CPU 编码器已经保存的 SSL、text 输出，并组合成四组输入。参考条件 `ge512` 完全相同。官方路径直接调用固定提交的 MRTE 模块，通过 hooks 保存内部结果；MLX 路径使用现有 Conv1d、attention 算子，并核对诊断展开与原 `multihead` 调用逐位一致。
...
"$REF/.venv-official-macos/bin/python" harness/mrte_numerical_diagnosis.py \
  --references "$REF" \
