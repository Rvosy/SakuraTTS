...
`src/sakuratts/mlx_sovits_encoder.py` 从已验证的 [SoVITS 解码包](2026-09-19-sovits-package.md) 中读取 235 个 `enc_p` 张量和一个量化码本，共 59,729,920 字节 FP32 数据。运行端只依赖 MLX、NumPy 和标准库，没有导入 PyTorch、官方工程或 Lite。
...
`harness/mlx_sovits_encoder_replay.py --self-test` 完成 17 项检查，证据目录为 `runs/20260919T114713.192724Z-mlx-sovits-encoder-self-test-cpu/`，相对于参考目录。
...
"$REF/.venv-mlx-macos/bin/python" harness/mlx_sovits_encoder_replay.py \
