...
`harness/g2pw_dedup_diagnostic.py` 从固定提交提取并执行真实 `_prepare_data`、`prepare_onnx_input` 和 `_predict_with_sentence_dedup`。OpenCC `s2tw`、PyPinyin、词表、排除字集合和默认 16 字上下文均按官方设置。只有 `_predict` 被替换成数组记录器，返回的 `None` 只用于让包装方法完成，不代表模型预测。
...
/Users/beyondpower/Documents/Projects/SakuraTTS-References/.venv-official-macos/bin/python \
  harness/g2pw_dedup_diagnostic.py
```
...
```sh
/Users/beyondpower/Documents/Projects/SakuraTTS-References/.venv-official-macos/bin/python \
  harness/audit_g2pw_dependencies.py
```
