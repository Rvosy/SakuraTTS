...
日期：2026-09-19。`scripts/prepare_japanese_reference.py` 已用固定官方代码重新准备现有日文参考，五组输出数组与旧条件包逐字节相同。准备 worker 和独立离线复核进程均实际退出 0。
...
工具复用现有 `harness/prepared_reference.py` 与 `harness/prepared_acoustic.py`，生成现有 `sakuratts-prepared-reference-v1` 格式。没有修改原导出器或 reader。新包包含 `reference_phones`、`prompt_semantic`、`reference_bert`、`ge`、`ge512`；包加载时检查模型和参考身份、文件散列、数组结构及内容散列。
...
"$REFS/.venv-official-macos/bin/python" \
  /Users/beyondpower/Documents/Projects/SakuraTTS/scripts/prepare_japanese_reference.py \
  --references "$REFS" \
