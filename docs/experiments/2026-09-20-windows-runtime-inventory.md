...
新增 [只读清点脚本](../../harness/windows_runtime_inventory.py)，逐文件记录逻辑字节、SHA256、目录分类和文件身份。相同路径被多个输入目录覆盖时只计一次；硬链接另算一次文件内容，不把已共享的空间算作可节省体积。符号链接和 Windows reparse point 不跟随，单独报告。本轮 9,965 个文件中没有输入范围内的硬链接别名或跳过项。

运行目录来自 [安装说明](../setup-windows-nvidia.md) 和 `models/windows-sakura/runtime.json`。该配置选择 `gpt`、`sovits-onnx-v1`、`frontend-classic` 及 CPU 准备的五组 `references`。不计旧声学导出、plus 前端资源包、CUDA 对照参考和准备日志。
