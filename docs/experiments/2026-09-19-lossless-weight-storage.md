...
新包有恢复与 hash 校验开销，GPT FP64 Prefill 还会在每次请求中重读权重。对两条用户报告样例执行相同固定历史、1024 KV 容量、2 次预热和 5 次正常测量，得到以下结果：
...
两轮所有步的 logits 逐位相同，也通过官方原容差。正式计时没有插入每步 CPU logits 复制、诊断计时器或内存采样。计时外的 Prefill 分段记录显示，日文的权重读取、恢复和校验由 0.05577 s 增至 0.19477 s，计算与其它处理为 0.17585 s、0.17535 s。当前实现会在每次高精度 Prefill 中重新校验存储字节和展开字节；这一重复工作是下一项优化依据，尚未在本轮省略。
