...
日期：2026-09-19。状态：真实历史的采样对照通过；3 项 Top-p 边界仍不兼容，尚未接入自由生成。

新增 [NumPy FP32 采样模块](../../src/sakuratts/sampling.py)，其运行依赖只有 NumPy 和 Python 标准库。它用于核对官方概率处理、指数噪声抽样和非流式停止规则，不是 GPU 采样优化。单独导入模块已确认没有加载 PyTorch；开发 Harness 使用 PyTorch 执行官方对照。
...
"$TTS_REF_DIR/.venv-official-macos/bin/python" -u harness/sampling_equivalence.py \
