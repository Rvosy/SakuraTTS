...
`harness/windows_webui_graph_benchmark.py` 直接执行本机 WebUI 的模型加载及 `get_tts_wav`，只略过 Gradio UI 构建和服务器启动。权重路径记录重定向到输出目录，原模型和源码不变，网络连接被禁止。WebUI 自己设置的高进程优先级予以保留并记录。测量包括每次重新计算参考条件到完整 PCM 的全部过程，没有替它补做缓存优化。FP32 / FP16 各完成 13 次请求，Graph 确实捕获；源码哈希检查均通过。
...
```powershell
& D:\Project\sakura\tts\g50\runtime\python.exe -B `
  harness/windows_webui_graph_benchmark.py `
  --official-root D:\Project\sakura\tts\g50 `
  --character D:\Project\sakura\characters\Sakura `
