...
```powershell
.venv-windows-runtime\Scripts\python.exe harness/windows_gpt_precision.py `
...
完整请求使用 `harness/windows_nvidia_benchmark.py --config models/windows-sakura/runtime.json --gpt-precision fp16 --repeats 3 --no-memory-sampler --skip-idle-unload --idle-ms 0 --output 新目录`。资源轮改为 `--repeats 1 --skip-reference-switch --skip-random --idle-ms 500` 并移除 `--no-memory-sampler`。生命周期用 `harness/windows_failure_lifecycle.py --config models/windows-sakura/runtime.json --gpt-precision fp16 --output 新目录/result.json`。
