...
`harness/windows_gemv_probe.py` 分为记录和探针两步。记录步骤使用现有 FP16 `CUDAGPT` eager decode，依次注入原始官方 capture 的 token 历史，保存真实线性层输入及原 cuBLAS 输出。探针读取这些输入和模型包中的真实权重，逐个比较 cuBLAS、`warp4`、`warp8`。
...
```powershell
.venv/Scripts/python.exe -B harness/windows_gemv_probe.py --mode record `
...
```powershell
.venv-windows-runtime/Scripts/python.exe -B harness/windows_gemv_probe.py --mode record `
  --gpt models/windows-sakura/gpt `
  --capture outputs/windows-baseline/fp32-naive/diagnostic-neutral-short.npz `
  --reference models/windows-sakura/references-cuda/中性 `
...
.venv-windows-runtime/Scripts/python.exe -B harness/windows_gemv_probe.py --mode probe `
