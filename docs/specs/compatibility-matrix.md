...
|---|---|---|---|---|---|
| V2Pro：“朱雀院红叶” | 10 条中日及混合输入已生成，已保存中间结果 | 中、日固定样例多次生成，已保存中间结果 | Lite 有 3 项用户报告的失败；官方与 official_text 候选固定开头/助词经用户确认；ASR 16 条已完成，其他质量待验证 | GPT 转换完成；纯 FP32 日文长句单步超差；独立 FP64 Prefill + FP32 Decode 的 10 条固定历史通过，完整语音链路未验证 | 暂缓，未验证 |
...
| 中、日文单独短句与长句 | 官方均已生成并保存 trace；MLX 日文长句第 329 步超预设容差，其他三条数值通过 | 已定位 Prefill 舍入放大，FP64 Prefill 候选通过原容差；正常速度与语音质量待验 |
...
| 自有 NumPy 采样与非流式停止 | 268 个真实步骤及 8 个参数变体对照通过；Top-p 3 个合成边界未兼容 | 边界处理、自由生成和 GPU 采样；详见 [采样实验](../experiments/2026-09-19-sampling-semantics.md) |
