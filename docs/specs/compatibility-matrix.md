...
|---|---|---|---|---|---|
| V2Pro：“朱雀院红叶” | 10 条中日及混合输入已生成，已保存中间结果 | 中、日固定样例多次生成，已保存中间结果 | Lite 有 3 项用户报告的失败；官方与 official_text 候选固定开头/助词经用户确认；ASR 16 条已完成，其他质量待验证 | 高精度 GPT 10 条固定历史及两条共享噪声的自有历史生成通过；独立声学编码器两条通过，完整语音链路未验证 | 暂缓，未验证 |
...
| 自有声学转换与编码器 | 650 张量解码包在官方 CPU 重载后波形逐位相同；独立 MLX 码本及 enc_p 两条、8 阶段通过 | reverse flow、声码器、完整波形及扩展语料；见 [编码器实验](../experiments/2026-09-19-mlx-sovits-encoder.md) |
