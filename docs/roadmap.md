...
5. 下一阶段的主要收益需在 Windows / NVIDIA 上确定：先固定 GPU / 驱动，重放现有验收包、建立官方同能力基线，再核对 KV 实际更新、主机传输、采样和声学工作区。依据结果选择一条生产后端，独立评估 FP16 与量化。CUDA Graph、Tensor Core 布局、TensorRT tactic、专用 CUDA kernel 和独显峰值留待实机，不继续深挖 Metal 专用调优。
