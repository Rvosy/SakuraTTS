...
from sakuratts.reference_condition import ARRAY_DTYPES, PreparedReference, sha256_array, sha256_file
...
def run(args):
    ...
    shutil.copy2(__file__, output / "source/harness/reference_package_replay.py")
    shutil.copy2(PROJECT / "src/sakuratts/reference_condition.py", output / "source/src/sakuratts/reference_condition.py")
    relocated = output / "relocated-package"
    relocated.mkdir()
    for name in ("manifest.json", "conditions.npz"):
        shutil.copy2(args.package / name, relocated / name)
    command = [sys.executable, str(output / "source/harness/reference_package_replay.py"), "--worker",
    ...
    report = {"status": "passed" if result.returncode == 0 else "failed", "command": command,
              ...
              "reader_sha256": sha256_file(output / "source/src/sakuratts/reference_condition.py"),
              "harness_sha256": sha256_file(output / "source/harness/reference_package_replay.py")}
