...
def main():
    ...
    sources = {}
    files = [*sorted((PROJECT / "harness").glob("*.py")), *sorted((PROJECT / "src/sakuratts").glob("*.py")),
             PROJECT / "harness/cases/speech_regressions.json"]
    for path in files:
    ...
    command = [sys.executable, "-u", str(run / "source/harness/official_reference_capture.py"), "--worker-run", str(run)]
    started = time.perf_counter()
