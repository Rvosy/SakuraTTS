"""Save official tokenizer outputs, then replay without Torch or Transformers.
...
def pure_g2pw(source):
    ...
    namespace["packing_arguments"] = dict(labels=labels, char2phonemes=char2phonemes,
                                         chars=sorted(char2phonemes), use_mask=True)
    return namespace
...
def evaluate(tokenizer, encode_features, cases, functions):
    ...
    for index, case in enumerate(cases):
        ...
        char2phonemes = functions["packing_arguments"]["char2phonemes"]
        positions = [i for i, character in enumerate(text) if character in char2phonemes]
        query_ids = sorted({positions[0], positions[len(positions) // 2], positions[-1]}) if positions else []
        ...
        if query_ids:
            packed = functions["prepare_onnx_input"](
                tokenizer, **functions["packing_arguments"], texts=[text] * len(query_ids), query_ids=query_ids)
            if len(packed) != 6:
...
def prepare(args):
    ...
    report = dict(status="prepared", **runtime_info(), official_commit=official_commit,
        scope="Tokenizer interfaces only. Actual G2PW functions extracted by AST; no neural model is constructed.",
    ...
    shutil.copy2(Path(__file__), run / "prepare-harness.py")
...
def validate(args):
    ...
    functions = pure_g2pw(run / "official-g2pw")
    ...
    shutil.copy2(Path(__file__), output / "validation-harness.py")
    shutil.copy2(PROJECT / "src/sakuratts/tokenizer.py", output / "tokenizer.py")
