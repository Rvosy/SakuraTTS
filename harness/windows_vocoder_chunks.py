...
from sakuratts.ort_sovits import INPUT_NAMES, ORTSoVITS, _package_file, read_manifest
...
def main():
    ...
    report = {"status": "running", "execution": args.execution, "mode": args.mode,
        ...
        "source_sha256": {name: sha256_file(ROOT/name) for name in
            ("harness/windows_vocoder_chunks.py", "harness/windows_acoustic_precision.py",
             "src/sakuratts/ort_sovits.py", "src/sakuratts/synthesis.py",
             "src/sakuratts/cuda_runtime.py", "scripts/windows_official_baseline.py")},
    ...
    try:
        ...
        if args.execution == "chunked":
            ...
            report["source_sha256"]["scripts/vocoder_receptive_field.py"] = sha256_file(ROOT/"scripts/vocoder_receptive_field.py")
