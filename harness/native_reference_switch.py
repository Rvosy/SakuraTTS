...
def main():
    ...
    case = next(row for row in read_json(PROJECT / "harness/cases/speech_regressions.json")["cases"] if row["id"] == CASE)
    run = Path(config["references"]) / "runs" / (datetime.now(timezone.utc).strftime("%Y%m%dT%H%M%S.%fZ") + "-native-reference-switch")
    run.mkdir(parents=True, exist_ok=False)
    files = [PROJECT / "harness/cases/speech_regressions.json", *sorted((PROJECT / "src/sakuratts").glob("*.py")),
             *sorted((PROJECT / "harness").glob("*.py"))]
    hashes = {}
    for path in files:
    ...
    for name in [*SEQUENCES, *(["official-b-replay"] if args.official_b_run else [])]:
        command = [sys.executable, "-u", str(run / "source/harness/native_reference_switch.py"), "--worker-run", str(run), name]
        started = time.perf_counter()
