...
def make_model(package, manifest_hash, softmax):
    ...
    from sakuratts.mlx_sovits_flow import MLXSoVITSFlow
...
def run(args):
    ...
    files = ["harness/sovits_reference_projection.py", "harness/portable_validation.py",
             *[f"src/sakuratts/{name}.py" for name in ("mlx_sovits", "mlx_sovits_encoder",
               "mlx_sovits_flow", "mlx_sovits_decoder", "sovits_package", "weight_storage", "synthesis",
               "generation", "sampling", "reference_condition")]]
    for name in files:
    ...
    for worker_name in ("diagnostic-baseline", "diagnostic-cached", "paired"):
        ...
        command = [sys.executable, str(run / "source/harness/sovits_reference_projection.py"),
