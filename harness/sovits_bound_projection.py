...
def run(args):
    ...
    files = sorted(set(prior["source_sha256"]) | {"harness/sovits_bound_projection.py", "src/sakuratts/mlx_gpt.py", "src/sakuratts/gpt_prefill.py"})
    for name in files:
    ...
    processes = []
    for name in workers:
        command = [sys.executable, str(output / "source/harness/sovits_bound_projection.py"),
