...
class ReferenceApiTests(unittest.TestCase):
    ...
    def test_same_audio_reuses_condition_but_rebuilds_text_and_language(self):
        with tempfile.TemporaryDirectory() as folder:
            ...
            english = cache.resolve(root / "audio.wav", "Hello", "en")
            self.assertEqual(segment.call_args.args, ("Hello.", "en"))
            self.assertEqual(english.manifest["identity"]["reference_language"], "en")
            self.assertEqual(first.manifest["identity"]["reference_language"], "ja")
            self.assertIs(first.prompt_semantic, english.prompt_semantic)
