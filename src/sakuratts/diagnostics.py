...
def check_windows_packages(config_path):
    ...
    if not references or config.get("default_reference", next(iter(references))) not in references:
        raise ValueError("At least one reference and a valid default reference are required")
